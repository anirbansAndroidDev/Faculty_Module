...
public class Carousel extends CarouselSpinner implements
	...
	protected boolean getChildStaticTransformation(View child,
		...
		// Center of the item
		float centerX = (float) child.getWidth() / 2, centerY = (float) child
				.getHeight() / 2;
		...
		public void SetImages(TypedArray array, boolean reflected) {
			...
			for (int i = 0; i < array.length(); i++) {
				...
				if (reflected) {
					...
					String Caption = "";
					CarouselItems carouselCnt = CarouselItems.values()[i];
					switch (carouselCnt) {
					case syncing_icon:
						Caption = "Sync Data";
						break;
					...
					// float blackTextWidth = mTextPaint.measureText(Caption);
					// float sentenceWidth = mTextPaint.measureText(Caption);
					...
					canvas.drawText(Caption, 2, 0, mTextPaint);
				...
				// Drawable imageGlow = getResources().getDrawable(
				// R.drawable.button_edge_glow);

				// imageView.setBackgroundDrawable(imageGlow);
