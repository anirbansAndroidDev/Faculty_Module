...
public class ExamDownloaderService {
	...
	public TaskStatusMsg ChapterSync(String s_user, String s_password) {
		...
		info.setTaskDone(UploadTask.BatchList);
		info.setStatus(-1);
		...
		if (soapResult != null && db!=null) {
			...
			if (loginStatus.equals("F")) {
				...
				try {
					...
					// downloadImage(getStudentPhotoUrl(s_user));
		...
		try {
			...
			if (soapResult != null) {
				...
				db.execSQL("DELETE FROM " + DatabaseHelper.TBL_CHAPTER
						+ " WHERE " + DatabaseHelper.FLD_BATCH_ID + " = "
						+ s_subject_id);
	...
	private long createSubject(String id_subject, String name_subject) {
		...
		try {
			...
			values.put(DatabaseHelper.FLD_BATCH_ID, id_subject);
			values.put(DatabaseHelper.FLD_BATCH_NAME, name_subject);

			lreturn = db.insert(DatabaseHelper.TBL_BATCH, null, values);
		} catch (SQLiteException sqle) {
			lreturn = -99;
		} catch (Exception e) {
			lreturn = -99;
	...
	private long createChapter(String id_subject, String id_chapter,
		...
		try {
			ContentValues values = new ContentValues();
			values.put(DatabaseHelper.FLD_BATCH_ID, id_subject);
			values.put(DatabaseHelper.FLD_CHAPTER_ID, id_chapter);
			values.put(DatabaseHelper.FLD_CHAPTER_NAME, name_chapter);
